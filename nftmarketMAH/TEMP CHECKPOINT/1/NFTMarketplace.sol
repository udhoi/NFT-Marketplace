...
contract NFTMarketplace is ReentrancyGuard, ERC721URIStorage, Ownable{
    ...
    function listNFT(uint256 tokenId, uint256 price, uint256 listFee, address tokenAddress) public payable nonReentrant{
        ...
        if(tokenAddress==address(0)){
            ...
            console.log(_customToken.approve(_scAddress, listFee));
            console.log(_customToken.allowance(msg.sender, address(this)));
            console.log(_customToken.balanceOf(msg.sender));
            console.log(_customToken.balanceOf(marketOwner));
            //IERC20 token = IERC20(_customTokenAddress);
            //console.log(token.balanceOf(msg.sender));
            //console.log(listFee);
            _customToken.transferFrom(msg.sender, marketOwner, listFee);
            console.log(_customToken.allowance(msg.sender, address(this)));
        }
    }
    // function giveApproval(uint256 listFee) public{
    //     bool passedOrFail;
    //     passedOrFail = _customToken.approve(_scAddress,listFee);
    //     console.log("Approve Passed?: ");
    //     console.log(passedOrFail);
    // }
    // function checkAllowance()public view returns(uint256){
    //     return _customToken.allowance(msg.sender,_scAddress);
    // }
