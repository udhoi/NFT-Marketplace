...
contract NFTMarketplace is ReentrancyGuard, ERC721URIStorage{
    ...
    /*
    Counters.Counter private totalUserWallets;
    ...
    //OLD VERSION_1
    mapping(uint256 => userWallet) private userMPWallets;
    ...
    */
    ...
    //OLD VERSION_1
    //Function transfers funds from User wallet (metamask) to the MP admin wallet.
    //NOTE:
    //COMPLETED (1) Must also create a variable that stores total userMPWallets value so that you can minus it from the admin wallet for the frontend.
    //COMPLETED (2) Must also create a structmap to store a list of all userMPWallets
    function userWtoMPWalletID(address gotAddress) public payable nonReentrant{
        
        uint256 currentNoUserMPWallets = totalUserWallets.current();

        //add this if old user
        bool oldUser = false;
        for(uint256 i=0; i<=currentNoUserMPWallets; i++){
            
            if(userMPWallets[i].userWalletAddress == gotAddress){
                userMPWallets[i].userWalletBalance = userMPWallets[i].userWalletBalance + msg.value;
        ...
        //add this if new user
        if(oldUser == false){
            totalUserWallets.increment();
            currentNoUserMPWallets = totalUserWallets.current();
            userMPWallets[currentNoUserMPWallets].userWalletAddress = gotAddress;
            userMPWallets[currentNoUserMPWallets].userWalletBalance = msg.value;
            payable(owner).transfer(msg.value);
        }
    ...
    */
    ...
    function userWtoMPWallet(address gotAddress) public payable nonReentrant{
    ...
    //Function to withdraw funds from the admin wallet to the user wallet
    //NOTE: Need to also remove this value from the global userMPWallets variable value.
    function mpWtoUserW(address userWallet, uint256 withdrawAmount) public payable nonReentrant{

        require(mpWallets[userWallet] <= withdrawAmount, "Wallet has lesser funds than asking withdraw amount.");
        mpWallets[userWallet] = mpWallets[userWallet] - msg.value;
        payable(userWallet).transfer(withdrawAmount);

    }

    //Function to check userWallet structmap before passing bid for users
    function bidCheck(address userWallet, uint256 currBid, uint256 tokenID) public view returns(bool){

        require((userWallet == MarketItemDatabase[tokenID].nftCreator && MarketItemDatabase[tokenID].nftOwner != address(0)) && userWallet != MarketItemDatabase[tokenID].nftOwner, "NFT owner is not allowed to bid!");
    ...
    //Function to transfer NFT from user to user
    //NOTE: After NFT is transfered you need to deduct the NFT price from the total userMPWallets
    function bidWon(address winner, uint256 bidAmount, uint256 nftOwnerGets, uint256 ownerGets, uint256 tokenId) public payable nonReentrant{

        require(MarketItemDatabase[tokenId].forSale == true, "NFT not listed for sale.");
        require(msg.value == bidAmount, "Please send exact NFT bid amount.");
        setApprovalForAll(_scAddress, true);
